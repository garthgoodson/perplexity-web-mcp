...
then packages everything into perplexity-web-mcp.mcpb (a zip file).
...
def build_mcpb() -> None:
    """Build the .mcpb file (zip containing manifest.json + run_server.py)."""
    version = get_version_from_pyproject()
    output = PROJECT_ROOT / f"perplexity-web-mcp-{version}.mcpb"

    print(f"\n📦 Building perplexity-web-mcp-{version}.mcpb\n")

    # Sync version
    sync_manifest_version(version)

    # Clean up old .mcpb files
    for old in PROJECT_ROOT.glob("perplexity-web-mcp*.mcpb"):
        if old != output:
            old.unlink()
